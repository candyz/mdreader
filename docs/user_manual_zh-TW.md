...
| **`o`** | **開啟檔案選擇器 (File Picker)**，隨時切換不同文件（支援隱藏檔切換與路徑記憶） |
| **`Ctrl+O`** | **Midnight Commander 雙欄檔案管理模式**：左右雙欄 (`Tab`)、`Ins`/`Space` 批次多選、`F3` 檢視、`F4` 編輯、`F5` 複製、`F6` 移動/重命名、`F7` 新增目錄、`F8` 刪除 |
...
| **`↓`** / **`↑`** | 逐行平滑向上 / 向下垂直捲動 |
| **`←`** / **`→`** / **`d`** / **`f`** | **向左 / 向右水平捲動**（`d` 向左、`f` 向右，當代碼區塊、表格或圖表太寬超出螢幕時使用） |
